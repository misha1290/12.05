...
console.log(getUserNames(chuVaki));

function getUsersByEyeColor(users, color) {
    return users.filter(user => user.eyeColor === color);
}
console.log(getUsersByEyeColor(chuVaki, 'blue'));

function getUserNamesByGender(users, gender) {
    return users.filter(user => user.gender === gender).map(user => user.name);
}
console.log(getUserNamesByGender(chuVaki, 'male'));

function getInactiveUsers(users) {
    return users.filter(user => !user.isActive);
}
console.log(getInactiveUsers(chuVaki));

function getUserByEmail(users, email) {
    return users.find(user => user.email === email);
}
console.log(getUserByEmail(chuVaki, 'sharlenebush@tubesys.com'));

function getUsersByAgeRange(users, minAge, maxAge) {
    return users.filter(user => user.age >= minAge && user.age <= maxAge);
}
console.log(getUsersByAgeRange(chuVaki, 30, 40));

function getTotalBalance(users) {
    return users.reduce((total, user) => total + user.balance, 0);
}
console.log(getTotalBalance(chuVaki));
